...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($data['type']) || empty($data['boundary_polygon']) || empty($data['total_area_acres'])) {
        http_response_code(400);
        echo json_encode(["error" => "type, boundary_polygon, and total_area_acres are required"]);
        exit;
    }
    ...
    try {
        ...
        // Insert new farm
        $stmt = $pdo->prepare("
            INSERT INTO farms (farmer_id, type, boundary_polygon, status, total_area_acres) 
            VALUES (:farmer_id, :type, ST_GeomFromText(:polygon, 4326), 'pending', :acres)
        ");
        $stmt->execute([
    ...
    try {
        ...
        if ($user['role'] === 'admin') {
            $stmt = $pdo->query("
                SELECT f.id, f.farmer_id, f.type, ST_AsText(f.boundary_polygon) as boundary_polygon, f.status, f.total_area_acres, f.created_at,
